...
#INCLUDE "TBICONN.CH"
#INCLUDE 'TOPCONN.CH'
//06-Ago-2021 Ronald Piscioneri: versao em uso ate Ago/2021, sem o PE01NFESEFAZ
/*
|=================================================================================
|   COMENTARIO
|---------------------------------------------------------------------------------
|	DIRETIVAS QUE IRAO IDENTIFICAR AS CUSTOMIZACOES NO FONTE NFESEFAZ, FACILITAN-
|	DO FUTURAS ATUALIZACOES
|
|	Edson Hornberger - 17/04/2015
|=================================================================================
*/
#IFNDEF TOTVS_NFESEFAZ
	#DEFINE CUSTOM_NFESEFAZ
	/*
	|---------------------------------------------------------------------------------
	|	Máximo de caracteres por linha de dados adicionais
	|---------------------------------------------------------------------------------
	*/
	#DEFINE MAXMENLIN 087
#ENDIF
...
//Alimentação do Grupo de Repasse
	...
	#IFDEF CUSTOM_NFESEFAZ
		...
		Local nI        := 0
		Local cAliasQry  := ''
		Local aPedCli    := {}  //Numero dos pedidos de compra dos clientes
		Local nPC        := 0
		Local aMensCli   := {}
		Local cMensST    := ""
		Local nPos       := 0
		Local _cEndCompleto
		Local _cZAK_ASSINATURA := ""
		Local cAliqInterna:= GetNewPar("MV_ESTICM","")
		Local cAliasAuxSd2			// Definição: desenv022
		Local cQuery	:= ""   // Definicao desenv031
		Local cCodCli	:= "" // Definicao desenv031
		Local cMsgSenado	:= "" // Definicao desenv032
		Local cMsgSINEF	:= "" // Definicao desenv034
		Local cMV_ESTADO	:= GetMv('MV_ESTADO')
		Local cFrmSenado	:= GETNEWPAR("TF_FRMSENA","DD7")
		Local cFrmSINEF	:= GETNEWPAR("TF_FRMSINE","F12")
		Local nTFart274	:= 0
		Local nPosSTentrada
		Local cCliLeiZFM 	:= GetNewPar("TF_LEIZFM","")	// Lista de clientes que quebram a regra do parametro MV_NFLEIZF
		Local nTFaliqIcm	:= 0
		Local cFrmFrete	:= GETNEWPAR("TF_FRMFRET","A84") // Formula contendo mensagem solicitada pela Logistica
		LOCAL NPOSNUMOLD	:= 0
		LOCAL NCLINUMOLD	:= 0
		LOCAL _lPedidoCrm 	:= .T. 
		LOCAL _nX			:= 0
		...
		Private mNotaSai	:= {}
		Private _cInfoDt	:= GETNEWPAR("TF_DTSAINF","0102|0102|0301|0303|0401|0404")
		Private _cICMSant	:= GETNEWPAR("TF_ICMSANT","N")
		Private aSTEntr    := {}
		Private _cSF2NumRm:= ""
		Private cAglutinaCGC	:= ""
	...
	#ENDIF // CUSTOM_NFESEFAZ

	Private cIntermediador:= ""
	Private cIndIntermed	:= ""
	...
	If cTipo == "1"
		...
		For nCountNF := 1 To Len(aCupRefLoj)
			...
			If MsSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)
				...
				ElseIf FunName() == "SPEDNFSE"
					...
					/*
					|---------------------------------------------------------------------------------
					|	Implementar no campo endereço os campos logradouro e numero
					|---------------------------------------------------------------------------------
					*/
						_cEndCompleto := SA1->A1_END
						If !Empty(SA1->A1_LOGR)
							_cEndCompleto := Alltrim(SA1->A1_LOGR) + " " + SA1->A1_END + ", " + SA1->A1_NUME
						EndIf

						aadd(aDest,AllTrim(SA1->A1_CGC))
						aadd(aDest,SA1->A1_NOME)
						aadd(aDest,FisGetEnd(_cEndCompleto,SA1->A1_EST)[1])
						If "/" $ FisGetEnd(_cEndCompleto,SA1->A1_EST)[3]
							aadd(aDest,IIF(FisGetEnd(_cEndCompleto,SA1->A1_EST)[3]<>"",FisGetEnd(_cEndCompleto,SA1->A1_EST)[3],"SN"))
						Else
							aadd(aDest,IIF(FisGetEnd(_cEndCompleto,SA1->A1_EST)[2]<>0,FisGetEnd(_cEndCompleto,SA1->A1_EST)[2],"SN"))
						EndIf
						aadd(aDest,FisGetEnd(_cEndCompleto,SA1->A1_EST)[4])
				...
				Else
					...
					If !SF2->F2_TIPO $ "DB"
						...
						/*
						|---------------------------------------------------------------------------------------------------
						|	Carrega variavel para indicar se haverá aglutinação (soma quantidades) de produtos iguais
						|---------------------------------------------------------------------------------------------------
						*/
							If SF2->(FieldPos("F2_NUMRM"))<>0
								_cSF2NumRm := Alltrim(SF2->F2_NUMRM)
							Else
								_cSF2NumRm := ""
							EndIf
						...
						/*
						|---------------------------------------------------------------------------------
						|	Desativa o uso da TAG <vDesc> como por exemplo o cliente CARREFOUR MANAUS
						|---------------------------------------------------------------------------------
						*/
							If At( SF2->F2_CLIENTE+SF2->F2_LOJA , cCliLeiZFM ) != 0
								lMvNFLeiZF := .T.
							EndIf
						...
						If SF2->(FieldPos("F2_CLIENT"))<>0 .And. !Empty(SF2->F2_CLIENT+SF2->F2_LOJENT) .And. (SF2->F2_CLIENT+SF2->F2_LOJENT<>SF2->F2_CLIENTE+SF2->F2_LOJA .OR. cTpOper == "1" )
							...
							If MsSeek(xFilial("SA1")+SF2->F2_CLIENT+SF2->F2_LOJENT)
								...
								*/
									_cEndCompleto := SA1->A1_END
									If !Empty(SA1->A1_LOGR)
										_cEndCompleto := Alltrim(SA1->A1_LOGR) + " " + Alltrim(SA1->A1_END) + ", " + SA1->A1_NUME
									EndIf
									aadd(aEntrega,SA1->A1_CGC)
									aadd(aEntrega,MyGetEnd(_cEndCompleto,"SA1")[1])
									aadd(aEntrega,ConvType(IIF(MyGetEnd(_cEndCompleto,"SA1")[2]<>0,MyGetEnd(_cEndCompleto,"SA1")[2],"SN")))
									aadd(aEntrega,MyGetEnd(_cEndCompleto,"SA1")[4])
					...
					*/
						...
						*/
									...
									aadd(aDest,AllTrim(SA1->A1_CGC))
									aadd(aDest,SA1->A1_NOME)
									aadd(aDest,MyGetEnd(_cEndCompleto,"SA1")[1])

									If MyGetEnd(_cEndCompleto,"SA1")[2]<>0
										aadd(aDest,MyGetEnd(_cEndCompleto,"SA1")[3])
									Else
										aadd(aDest,"SN")
									EndIf

									aadd(aDest,IIF(SA1->(FieldPos("A1_COMPLEM")) > 0 .And. !Empty(SA1->A1_COMPLEM),SA1->A1_COMPLEM,MyGetEnd(_cEndCompleto,"SA1")[4]))
							...
							Else
								...
								#IFDEF CUSTOM_NFESEFAZ
									...
									_cEndCompleto := SA2->A2_END
									If !Empty(SA2->A2_LOGR)
										_cEndCompleto := Alltrim(SA2->A2_LOGR) + " " + SA2->A2_END + ", " + SA2->A2_NR_END
									EndIf

									aadd(aDest,MyGetEnd(_cEndCompleto,"SA2")[1])

									If MyGetEnd(_cEndCompleto,"SA2")[2]<>0
										aadd(aDest,MyGetEnd(_cEndCompleto,"SA2")[3])
									Else
										aadd(aDest,"SN")
									EndIf

									aadd(aDest,IIF(SA2->(FieldPos("A2_COMPLEM")) > 0 .And. !Empty(SA2->A2_COMPLEM),SA2->A2_COMPLEM,MyGetEnd(_cEndCompleto,"SA2")[4]))
					...
					/* Customização para atender demanda da ASTEC e EBCT - Na TAG da razão social da NFe deverá contemplar o codigo da transportadora */
								#IFDEF CUSTOM_NFESEFAZ
									SD2->(DbSetOrder(3))
									...
									If SC5->C5_CLASPED = "A" .AND. (SF2->F2_TPFRETE == "S"  .OR. SC5->C5_TPFRETE == "S") .AND. !SD2->(Eof()) .AND. SA4->(Fieldpos("A4__NOMASS")) > 0
										If .NOT. EMPTY(SA4->A4__NOMASS)
											aadd(aTransp,SA4->A4__NOMASS)
										Else
							...
							#IFDEF CUSTOM_NFESEFAZ
								If SD2->(FieldPos("D2_XMENSNF"))<>0 //Mensagem de exceção fiscal
									cField  +=",D2_XMENSNF"
								EndIf
							#ENDIF
		...
		//						If MsSeek(xFilial("SD2")+(cAliasSD2)->D2_NFORI+(cAliasSD2)->D2_SERIORI+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_COD+(cAliasSD2)->D2_ITEMORI)
						...
						MsSeek(xFilial("SC6")+(cAliasSD2)->D2_PEDIDO+(cAliasSD2)->D2_ITEMPV+(cAliasSD2)->D2_COD)
							...
							*/
										If !Empty(SC5->C5_MENNOTA)
											If .NOT. (cempant = "03" .and. cfilant = "02" .and.  .not. EMPTY(_cSF2NumRm)) /*Quando NFe de cross docking não imprime a mensagem dos pedidos*/
												If AT("|", SC5->C5_MENNOTA) > 0
													_aLinhas := StrToArray(AllTrim(SC5->C5_MENNOTA),"|")
												Else
													_aLinhas := {AllTrim(SC5->C5_MENNOTA)}
												EndIf
											Else
												_aLinhas := {}
											EndIf
										...
										For nZZ := 1 to Len( _aLinhas )
											If AScan(aMensCli, AllTrim(_aLinhas[nZZ])) == 0
												AAdd(aMensCli, AllTrim(_aLinhas[nZZ]))
											EndIf
										Next nZZ
										
										NCLINUMOLD := AScan(aMensCli, ALLTRIM("PEDIDO PORTAL: " + SC5->C5_NUMOLD) )
										IF NCLINUMOLD > 0
											NPOSNUMOLD := AT( ALLTRIM("PEDIDO PORTAL: " + SC5->C5_NUMOLD) , aMensCli[NCLINUMOLD])
											IF NPOSNUMOLD !=0
												NPOSNUMOLD := NPOSNUMOLD + LEN(ALLTRIM("PEDIDO PORTAL: " + SC5->C5_NUMOLD))
												aMensCli[NCLINUMOLD] := SUBSTRING(aMensCli[NCLINUMOLD],1,NPOSNUMOLD ) + " __" +SUBSTRING(aMensCli[NCLINUMOLD], NPOSNUMOLD+1 , LEN(aMensCli[NCLINUMOLD]))
											ENDIF
										ELSE
											IF .NOT. EMPTY(SC5->C5_NUMOLD) 
												_lPedidoCrm := .T. 
												...
												Next _nX
												IF _lPedidoCrm
													AAdd(aMensCli, ALLTRIM("PEDIDO PORTAL: " + SC5->C5_NUMOLD) + " __" )
												EndIf
											ENDIF
										ENDIF

										SE4->(dbseek(xFilial('SE4')+SC5->C5_CONDPAG))
										If !Empty(Alltrim(SE4->E4_DESCRI)) .and. AScan(aMensCli, 'Cond.Pagto: ' + Alltrim(SE4->E4_DESCRI)) == 0 .AND. Empty(_cSF2NumRm) // Empty(SC5->C5_NUMOC)
											AAdd(aMensCli, 'Cond.Pagto: ' + Alltrim(SE4->E4_DESCRI))
										Endif

										If !Empty(SC5->C5_MENPAD) .And. AScan(aMensCli, AllTrim(FORMULA(SC5->C5_MENPAD))) == 0
											AAdd(aMensCli, AllTrim(FORMULA(SC5->C5_MENPAD)))
										EndIf

										SM4->(DbSeek( xFilial("SM4") + SF4->F4_MSGPAD ))
										If !Empty( SM4->M4_FORMULA ) .and. Ascan(aMsgPadF4,SF4->F4_MSGPAD) == 0
											Aadd(aMsgPadF4,SF4->F4_MSGPAD)
											cRet:=Alltrim( SM4->M4_FORMULA )

											aTexto:=MontaTexto(cRet)

											For nI:=1 to Len(aTexto)
												If !Empty(aTexto[nI]) .And. AScan(aMensCli, AllTrim(aTexto[nI])) == 0
													AAdd(aMensCli, Alltrim(aTexto[nI]))
												EndIf
											Next

											AddMsgTxt(@aTextoAdic,@cTexto,'#',cAliasSD2,cTipo)  // Carrega as Mensagens com "ICMS,PIS E COFINS"
											If !Empty(cTexto)
												AAdd(aMensCli, cTexto)
											EndIf
										Endif

										If SD2->(FieldPos("D2_XMENSNF")) != 0
											SM4->(DbSeek( xFilial("SM4") + (cAliasSD2)->D2_XMENSNF ))
											If ! Empty(sm4->m4_formula) .and. aScan(_aMsgPadST, (cAliasSD2)->D2_XMENSNF) == 0
												aAdd(_aMsgPadST, (cAliasSD2)->D2_XMENSNF)
												cMensST += PADR(Formula((cAliasSD2)->D2_XMENSNF),MAXMENLIN)  //mjl-03/2
											Endif
										Endif
									...
									If !AllTrim(SC5->C5_MENNOTA) $ cMensCli .AND. 1=2
							...
							/*
							|---------------------------------------------------------------------------------
							|	Informacao sobre o numero do pedido de compra do cliente
							|---------------------------------------------------------------------------------
							*/
										If !Empty(SC6->C6_PEDCLI)
											If aScan(aPedCli, Alltrim(SC6->C6_PEDCLI)) == 0
							...
							/*
							|---------------------------------------------------------------------------------
							|	Implementação: 	Refere-se à TAG xPed para carregar no XML o numero do pedido
							|					do cliente no ite do produto
							|   Observação...: 	Ver e-mail da Selma assunto "Adição de campo na Nota Eletrônica"
							| 	Autor........: 	Carlos Alday Torres	- Data: 28/08/2013
							|---------------------------------------------------------------------------------
							*/
										IF !Empty(SC6->C6_PEDCLI)
											aadd(aPedCom,{AllTrim(SC6->C6_PEDCLI),0})
										ELSE
							...
							/*
							|---------------------------------------------------------------------------------
							|	Referente a inclusão do código do produto do cliente apos a descricao do
							|	produto no XML
							|   Data: 26/12/2012   Analista: Edson Hornberger
							|---------------------------------------------------------------------------------
							*/
										IF SELECT("TSA7") > 0
										...
										cquery += "COUNT(*) AS QTD "
										cQuery += "FROM "
										cQuery += RetSQLName("SA7") + " SA7 "
										cquery += "WHERE "
										cQuery += "SA7.A7_FILIAL = '" + XFILIAL("SA7") + "' AND "
										cQuery += "SA7.A7_CLIENTE = '" + SF2->F2_CLIENTE + "' AND "
										cQuery += "SA7.A7_LOJA = '" + SF2->F2_LOJA + "' AND "
										cQuery += "SA7.A7_PRODUTO = '" + (cAliasSD2)->D2_COD + "' AND "
										cQuery += "SA7.D_E_L_E_T_ = ' '"

										TCQUERY CQUERY NEW ALIAS "TSA7"
										DBSELECTAREA("TSA7")
										DBGOTOP()
										...
										IF TSA7->QTD > 0
											...
											TCQUERY CQUERY NEW ALIAS "TSA7"
											DBSELECTAREA("TSA7")
											CCODCLI := TSA7->A7_CODCLI

										ENDIF

							/*
							|---------------------------------------------------------------------------------
							|	Incluir mensagem DD7 para produtos importados em operações interestaduais,
							|	conforme chamado 00003270
							|---------------------------------------------------------------------------------
							*/
										If SB1->B1_ORIGEM = '2' .AND.   SA1->A1_EST != cMV_ESTADO   .AND.   .NOT. (SC5->C5_TIPO $ "DB")
											If SM4->(DbSeek( xFilial("SM4") + cFrmSenado ))
												cMsgSenado	:= Alltrim(SM4->M4_FORMULA) + " - "
											EndIf
										EndIf

							/*
							|---------------------------------------------------------------------------------
							|	Implementação: 	Refere-se a mensagem da SINEF. Ver e-mail da Patricia Lima
							|					09/05/2013 14:02
							|	Autor........: 	Carlos Alday Torres
							|---------------------------------------------------------------------------------
							*/
										If SB1->B1_ORIGEM != "0" .AND. cEmpAnt="01"  .AND. SB1->B1_TIPO="PA"
											If SM4->(DbSeek( xFilial("SM4") + cFrmSINEF ))
												cMsgSINEF	:= Alltrim(SM4->M4_FORMULA) + " "
											EndIf
										EndIf

										cCodProd  := (cAliasSD2)->D2_COD
										cDescProd := ALLTRIM(IIF(Empty(SC6->C6_DESCRI),SB1->B1_DESC,SC6->C6_DESCRI)) + " - " + IIF(!EMPTY(CCODCLI)," - " + ALLTRIM(CCODCLI),"")
										...
										If CDA->(ColumnPos("CDA_CODLAN")) > 0
											...
											If MsSeek(cSeekCDA) //CDA_FILIAL, CDA_TPMOVI, CDA_ESPECI, CDA_FORMUL, CDA_NUMERO, CDA_SERIE, CDA_CLIFOR, CDA_LOJA, CDA_NUMITE, CDA_SEQ, CDA_CODLAN, CDA_CALPRO
												While cSeekCDA == CDA->(CDA_FILIAL + CDA_TPMOVI + CDA_ESPECI + CDA_FORMUL + CDA_NUMERO + CDA_SERIE + CDA_CLIFOR + CDA_LOJA + CDA_NUMITE)
													If !Empty(CDA->CDA_CODLAN) .And. Len(AllTrim(CDA->CDA_CODLAN)) >= 8
									...
									#IFDEF CUSTOM_NFESEFAZ
										// Na matriz aProd elemento do codigo de barras foi implementada regra para atender os produtos sem EAN
									#ENDIF

									aadd(aProd,	{Len(aProd)+1,;
										cCodProd,;
										IIf(Val(SB1->B1_CODBAR)==0 .OR. SUBSTR(SB1->B1_CODBAR,1,LEN(ALLTRIM(SB1->B1_COD))) = ALLTRIM(SB1->B1_COD) .OR. LEN(ALLTRIM(SB1->B1_CODBAR))<13,"",StrZero(Val(SB1->B1_CODBAR),Len(Alltrim(SB1->B1_CODBAR)),0)),;
										cDescProd,;
									...
									#IFDEF CUSTOM_NFESEFAZ
										...
										// Mensagem referente ao ICMS-ST nas vendas
										// para o estado de SP - Empresa Mercabel/Matriz (02/01)
										// -----------------------------------------------------------
										If _cICMSant = "S" // cEmpAnt == '02' .And. cFilAnt == '01'
											//
											// A condição de lote na tabela SD2 não existe a partir de 07/10/2011
											//
											// Comentado em 14/10/2011 por Rodrigo e Carlos
											//If Rastro((cAliasSD2)->D2_COD) .And. !Empty((cAliasSD2)->D2_LOTECTL) .And.;
												//	(AllTrim((cAliasSD2)->D2_CF) == '5405' .Or. AllTrim((cAliasSD2)->D2_CF) == '5910')

											If (AllTrim((cAliasSD2)->D2_CF) == '5405' .Or. AllTrim((cAliasSD2)->D2_CF) == '5910')

												//--1o. busca a ultima entrada considerando
												//--o lote:
												cAliasQry := GetNextAlias()
												BeginSQL Alias cAliasQry
										SELECT	TOP 1 SD1.D1_FILIAL, SD1.D1_COD, SD1.D1_DTDIGIT,
										SD1.D1_QUANT, SD1.D1_BRICMS, SD1.D1_ICMSRET, SD1.D1_BASEICM, SD1.D1_VALICM
										, SD1.D1_MARGEM, SD1.D1_VUNIT
										FROM	%Table:SD1% SD1
										WHERE	SD1.D1_FILIAL  = %xFilial:SD1% AND
										SD1.D1_COD     = %Exp:(cAliasSD2)->D2_COD% AND
										SD1.D1_LOTECTL = %Exp:(cAliasSD2)->D2_LOTECTL% AND
										SD1.D1_BRICMS > 0 AND
										SD1.D1_ICMSRET > 0 AND
										SD1.%NotDel%
										ORDER
										BY		SD1.D1_FILIAL, SD1.D1_DTDIGIT DESC
												EndSQL

												(cAliasQry)->(DbGoTop())
												If (cAliasQry)->(Eof())
													//--Se nao encontrou a ultima entrada
													//--considerando o lote, busca a ultima
													//--entrada para o produto, sem considerar
													//--o numero do lote:
											...
											BY		SD1.D1_FILIAL, SD1.D1_DTDIGIT DESC
												...
												If !(cAliasQry)->(Eof())
													nX := AScan(aSTEntr, {|x| x[1] == (cAliasSD2)->D2_ITEM + (cAliasSD2)->D2_COD})

													nTFaliq:= Val( Substr(cAliqInterna , At( cMV_ESTADO , cAliqInterna )+2,2) )
													nTFart274 := ((cAliasQry)->(D1_BRICMS / D1_QUANT) * (cAliasSD2)->D2_QUANT)*(nTFaliq/100)
													nTFart274 -= (cAliasSD2)->D2_TOTAL*(nTFaliq/100)
													If nTFart274 < 0
														nTFart274:=0
													EndIf
													nTFaliqIcm := ( ( ((cAliasQry)->D1_ICMSRET / (cAliasQry)->D1_QUANT ) * (cAliasSD2)->D2_QUANT   ) / (cAliasSD2)->D2_TOTAL) * 100

													If nX == 0
														AAdd(aSTEntr, {	(cAliasSD2)->D2_ITEM + (cAliasSD2)->D2_COD,;
															((cAliasQry)->(D1_BRICMS / D1_QUANT)) * (cAliasSD2)->D2_QUANT,;
															((cAliasQry)->(D1_ICMSRET / D1_QUANT)) * (cAliasSD2)->D2_QUANT,;
															((cAliasQry)->(D1_BASEICM / D1_QUANT)) * (cAliasSD2)->D2_QUANT,;
															((cAliasQry)->(D1_VALICM / D1_QUANT)) * (cAliasSD2)->D2_QUANT,;
															nTFart274,;
															nTFaliqIcm  })

													Else
														//aSTEntr[nX, 2] += ((cAliasQry)->(D1_BRICMS / D1_QUANT)) * (cAliasSD2)->D2_QUANT
														aSTEntr[nX, 3] += ((cAliasQry)->(D1_ICMSRET / D1_QUANT)) * (cAliasSD2)->D2_QUANT
														aSTEntr[nX, 4] += ((cAliasQry)->(D1_BASEICM / D1_QUANT)) * (cAliasSD2)->D2_QUANT
														aSTEntr[nX, 5] += ((cAliasQry)->(D1_VALICM / D1_QUANT)) * (cAliasSD2)->D2_QUANT
														aSTEntr[nX, 6] += nTFart274
														aSTEntr[nX, 7] := nTFaliqIcm

													EndIf
										/*
										Autor: CT                            Data: 25/08/2015
										Implementado o cálculo da base de ICMs Solidário na entrada para buscar pela MARGEM (MVA), devido à divergencia
										no cálculo ocorrido no dia 19/08/2015, para os mesmos valores de preço unitário e margem o calculo ficou incorreto.
										Utilziar SCRIPT abaixo para entendimento:
										. (cAliasSD2)->D2_QUANT=3366
										SELECT (D1_BRICMS/D1_QUANT)*3366 AS VL_CALCULADO,D1_BRICMS,D1_EMISSAO AS EMISSAO,D1_ICMSRET,(D1_ICMSRET / D1_QUANT),D1_MARGEM,D1_DOC,*
										FROM SD1030 SD1
										WHERE D1_FILIAL='01'
										AND D_E_L_E_T_=''
										AND D1_EMISSAO>='20150817' AND D1_EMISSAO<='20150821'
										AND D1_COD='000100074'
										ORDER BY SD1.D1_FILIAL, SD1.D1_DTDIGIT DESC
										*/
													If (cAliasQry)->D1_MARGEM != 0
														nPosSTentrada := AScan(aSTEntr, {|x| x[1] == (cAliasSD2)->D2_ITEM + (cAliasSD2)->D2_COD})
														aSTEntr[ nPosSTentrada , 2] := ((cAliasQry)->D1_VUNIT * (cAliasSD2)->D2_QUANT ) * (1 + ((cAliasQry)->D1_MARGEM/100) )
													EndIf
												EndIf
												(cAliasQry)->(DbCloseArea())
											EndIf
				...
				// Msg Zona Franca de Manaus / ALC
					...
					dbSeek (xFilial("SF3")+SF2->F2_CLIENTE+SF2->F2_LOJA+SF2->F2_DOC+SF2->F2_SERIE)
						...
						SF2->F2_DOC == SF3->F3_NFISCAL .And. SF2->F2_SERIE == SF3->F3_SERIE
							...
							/*
							|---------------------------------------------------------------------------------
							|	Mensagem referente aos impostos praticados para ZFM/ALC
							|---------------------------------------------------------------------------------
							*/
																		AAdd(aMensCli, 'Numero do SUFRAMA: ' + SA1->A1_SUFRAMA)
																	#ENDIF
					...
					/*
					|---------------------------------------------------------------------------------
					|	Mensagem padrao - Solicitacao Sr. Edson (Fiscal)
					|---------------------------------------------------------------------------------
					*/
																AAdd(aMensCli, "Codigo Regime Tributario: 3-Regime Normal")
					/*
					|---------------------------------------------------------------------------------
					|	Mensagem referente ao No. do pedido de venda:
					|---------------------------------------------------------------------------------
					*/
																AAdd(aMensCli, "Numero do pedido: " + SC5->C5_NUM)

																If SM0->M0_CODIGO='05'
																	If xFilial("SF2")='01'
																		If Empty(_cZAK_ASSINATURA)
																			_cZAK_ASSINATURA := SC5->C5_OBSSERV
																		EndIf
																	EndIf
																EndIf
					...
					/*
					|---------------------------------------------------------------------------------
					|	Informacao sobre o pedido de compra do cliente
					|---------------------------------------------------------------------------------
					*/
																If Len(aPedCli) > 0
																	AAdd(aMensCli, "Num.Ped.Cliente: ")
																	For nPC := 1 To Len(aPedCli)
																		aMensCli[Len(aMensCli)] += aPedCli[nPC] + "|"
																	Next nPC
																EndIf
																aPedCli := {}
																nPC     := 0
					...
					/*
					|---------------------------------------------------------------------------------
					|	Implementação: 	Refere-se a mensagem da concessão no regime especial
					|	Autor........: 	Carlos Alday Torres - Data: 17/11/2011
					|---------------------------------------------------------------------------------
					*/
																If SM0->M0_CODIGO $ GetNewPar("TF_REGIMEE","")  // Empresas que participam do regime especial
																	...
																	cQuery += " 	INNER JOIN "+RetSqlName("SF4")+" SF4 WITH(NOLOCK) ON SF4.F4_CODIGO=SD2.D2_TES "
																	...
																	DBSelectArea("ZAL")
																	DbSetOrder(1)
																	If DbSeek( xFilial("ZAL") + SF2->F2_CLIENTE + SF2->F2_LOJA )

																		While !EOF() .and. ZAL->ZAL_FILIAL + ZAL->ZAL_CLIENT + ZAL->ZAL_LOJA = xFilial("ZAL") + SF2->F2_CLIENTE + SF2->F2_LOJA

																			If (((cAliasAuxSd2)->REGIME  !=  0 .and. ZAL->ZAL_TIPSIT = 'N') .or. ZAL->ZAL_TIPSIT = 'T') .and. SF2->F2_EMISSAO <= ZAL->ZAL_VALIDA  .and.  Empty( __cTexto )

																				__cTexto += AllTrim(Formula(ZAL->ZAL_MENSAG)) // Retirada as aspas de inicio e fim
					...
					/*
					|---------------------------------------------------------------------------------
					|	Montagem da mensagem p/ cliente
					|---------------------------------------------------------------------------------
					*/
																//cMensCli := ''
																For nX := 1 To Len(aMensCli)

						/*
																	If Len(AllTrim(aMensCli[nX])) <= MAXMENLIN
							cMensCli += PadR(AllTrim(aMensCli[nX]), MAXMENLIN)
																	Else
																		For nY := 1 To Len(aMensCli[nX]) Step 80
								cMensCli += PadR(SubStr(AllTrim(aMensCli[nX]), nY, MAXMENLIN), MAXMENLIN)
																		Next
																	EndIf
						*/
																	cMensCli += AllTrim(aMensCli[nX])
					...
					|---------------------------------------------------------------------------------
					Regra valida para os pedidos de vendas 
					*/
																IF SC5->C5_TIPO = "N" .AND. SC5->C5_INTER != "S" .AND. .NOT. EMPTY(cFrmFrete)
																	If SM4->(DbSeek( xFilial("SM4") + cFrmFrete ))
																		cMensCli += AllTrim(FORMULA(cFrmFrete)) + SPACE(02)
																	EndIf
																ENDIF
		    ...
		    //Retira o desconto referente ao RICMS 43080/2002
				...
				/*
				|---------------------------------------------------------------------------------
				|	Texto digitado no campo de observacoes da nota fiscal de entrada
				|---------------------------------------------------------------------------------
				*/
																			If SF1->F1_FORMUL="S" .And. !Empty(SF1->F1_OBSNFE)
...
//Geracao do arquivo XML
															If !Empty(aNota)
																...
																#IFDEF CUSTOM_NFESEFAZ
																	...
																	If cAglutinaCGC = aDest[01] .AND. ALLTRIM(_cSF2NumRm)="CROSS" // CNPJ do destinatario
																		Aglutina_aProd(@aProd,@aICMS,@aICMSST,@aIPI,@aPIS,@aPISST,@aCOFINS,@aCOFINSST,@aISSQN,@aCST,@aMed,@aArma,@aveicProd,@aDI,@aAdi,@aExp,@aPisAlqZ,@aCofAlqZ)
																	EndIf
																...
																#IFDEF CUSTOM_NFESEFAZ
																	...
																	For nX := 1 to Len(aTextoAdic)
																		cMensFis+= chr(13)+chr(10)+aTextoAdic[nX,2]+"R$ "+Alltrim(Trans(aTextoAdic[nX,3],"@E 999,999,999.99"))+" "
																	Next
																...
																cString += "</infNFe>"
															EndIf
															Memowrite("NFESEFAZ_nota_fiscal_40.XML",cString)
...
//pois todos os cálculos fiscais são feitos com base nessa informação e não no campo A1_TIPO.
	...
	If cVeramb >= "3.10"
		...
		cIndPres := retIndPres(aNota[04], aNota, aProd)
...
cString += '<qCom>'+ConvType(aProd[09],15,4)+'</qCom>'
	...
	#IFDEF CUSTOM_NFESEFAZ
		If	cAglutinaCGC = aDest[01] .AND. _cSF2NumRm="CROSS" // CNPJ do destinatario
			cString += '<vUnCom>'+ConvType(aProd[10]/aProd[12],21,4)+'</vUnCom>'
		Else
		...
		Endif
		/* Foi realizada consulta junto à controladoria a prossibilidade de manter os preços de transferencia (CROSS DOCKING) com duas casas decimais em 05/02/2016 */
		/* Para o produto 000100074 ocorre rejeição SEFAZ relativo ao valor total do produto para tanto foi implementada a regra especifica em 10/02/2016 */
		If	cAglutinaCGC = aDest[01] .AND. _cSF2NumRm="CROSS" .and. At(Alltrim(ConvType(aProd[02])) , "000100074") != 0
			nCtTotal := aProd[10]/aProd[12]
			nCtTotal := ( aProd[12] * nCtTotal ) 
			cString += '<vProd>'+ConvType( nCtTotal ,21,4)+'</vProd>'
		ElseIf	cAglutinaCGC = aDest[01] .AND. _cSF2NumRm="CROSS" //.and. At(Alltrim(ConvType(aProd[02])) , "000100075|000100079") != 0
			nCtTotal := round(aProd[10]/aProd[12],4)
			nCtTotal := ( aProd[12] * nCtTotal ) 
			cString += '<vProd>'+ConvType( nCtTotal ,21,2)+'</vProd>'
		Else
...
cString += '<uTrib>'+ConvType(aProd[11])+'</uTrib>'
	...
	Endif
	/* Devido ao arredondamento de 8 casas decimais na aglutinação foi implementa está condição para o projeto Cross Docking*/
	#IFDEF CUSTOM_NFESEFAZ
		If	cAglutinaCGC = aDest[01] .AND. _cSF2NumRm="CROSS" // CNPJ do destinatario
...
//Nota Técnica 2013/006
	...
	If  !lIssQn
		...
		/*matxfis de 27/08/15 (TRDESQ) passou a gravar o campo CD2_MODBC corretamente*/	
			#IFDEF CUSTOM_NFESEFAZ
			/* implementado case igual a 6 para atender vendas a consumidor final */
			//Case aICMSST[03] == "6"
			//	aICMSST[03] := "6"
			#ENDIF // CUSTOM_NFESEFAZ
			Do Case
			...
			//Se o parâmetro  MV_ULTAQUI que trata a última aquisição não estiver preenchido usa o modelo antigo de rastro.
				...
				If nBaseIcm > 0 .and. nValICM > 0	.and. nAlqICM > 0
					...
					// cMensFis += "Imposto Recolhido por Substituição - Artigo 274 do RICMS (Lei 6.374/89, art.67,Paragrafo 1o., e Ajuste SINIEF-4/93',cláusa terceira, na redação do Ajuste SINIEF-1/94) 'Cod.Produto:  " +ConvType(aProd[02])+" ' Valor da Base de ST: R$ " +str(nBaseIcm,15,2)+" Valor de ICMS ST: R$ "+str(nValICM,15,2)+" "
						If AT("Artigo 274 do RICMS",cMensFis) > 0
						cMensFis += " Cod.Produto: " +ConvType(aProd[02])+" ' Valor da Base de ST: R$ " +str(nBaseIcm,15,2)+" Valor de ICMS ST: R$ "+str(nValICM,15,2)+" "
						Else
						cMensFis += "Imposto Recolhido por Substituição - Artigo 274 do RICMS (Lei 6.374/89, art.67,Paragrafo 1o., e Ajuste SINIEF-4/93',cláusa terceira, na redação do Ajuste SINIEF-1/94) 'Cod.Produto: " +ConvType(aProd[02])+" ' Valor da Base de ST: R$ " +str(nBaseIcm,15,2)+" Valor de ICMS ST: R$ "+str(nValICM,15,2)+" "
						EndIf
						#IFDEF CUSTOM_NFESEFAZ
						cMensFis += "Valor da parcela do imposto retido: R$ " + IIF( (nBaseIcm*(18/100)) - (aProd[10]*(18/100)) > 0 , Alltrim(str((nBaseIcm*(18/100)) - (aProd[10]*(18/100)),15,2)),"0.00") + " "
						#ENDIF // CUSTOM_NFESEFAZ
	...
	Else
		If Len(aISSQN)>0 .and. !Empty(aISSQN[01])
			...
			If cVerAmb >= "3.10"
				...
				cString += NfeTag('<cmunInc>',ConvType(cMunPres))
...
if !empty(cIndIntermed) .and. date() >= cTod("01/09/2021")
...
=================================================================================
*/
Static Function Aglutina_aProd(aProd,aICMS,aICMSST,aIPI,aPIS,aPISST,aCOFINS,aCOFINSST,aISSQN,aCST,aMed,aArma,aveicProd,aDI,aAdi,aExp,aPisAlqZ,aCofAlqZ)
	...
	Local aPISSTAglut		:= {}
	Local aCOFINSAglut	:= {}
	Local aCOFINSSTAglut	:= {}
	Local aISSQNAglut		:= {}
	Local aCSTAglut		:= {}
	Local aMedAglut		:= {}
	Local aArmaAglut		:= {}
	Local aveicProdAglut	:= {}
	Local aDIAglut		:= {}
	Local aAdiAglut		:= {}
	Local aExpAglut		:= {}
	Local aPisAlqZAglut	:= {}
	...
	For nX := 1 to Len(aProd)
		...
		//nPosProd := Ascan(aProdAglut,{|x| x[2] = aProd[nX,2] .And. x[7] = aProd[nX,7] .And. Round(x[16],2) = Round(aProd[nX,16],2) .And. x[33] = aProd[nX,33] .And. x[23] = aProd[nX,23]})
		nPosProd := Ascan(aProdAglut,{|x| x[2] = aProd[nX,2] .And. x[7] = aProd[nX,7] .And. x[33] = aProd[nX,33] .And. x[23] = aProd[nX,23]})
		If Empty(nPosProd)

			Aadd(aProdAglut,Array(Len(aProd[nX])))
			aProdAglut[Len(aProdAglut)]   := Aclone(aProd[nX])
			aProdAglut[Len(aProdAglut),1] := nSeq

			Aadd(aICMSAglut,Array(Len(aICMS[nX])))
			aICMSAglut[Len(aICMSAglut)]   := Aclone(aICMS[nX])

			Aadd(aICMSSTAglut,Array(Len(aICMSST[nX])))
			aICMSSTAglut[Len(aICMSSTAglut)]   := Aclone(aICMSST[nX])

			Aadd(aIPIAglut,Array(Len(aIPI[nX])))
			aIPIAglut[Len(aIPIAglut)]   :=  Aclone(aIPI[nX])

			Aadd(aPISAglut,Array(Len(aPIS[nX])))
			aPISAglut[Len(aPISAglut)]   :=  Aclone(aPIS[nX])

			Aadd(aPISSTAglut,Array(Len(aPISST[nX])))
			aPISSTAglut[Len(aPISSTAglut)]   :=  Aclone(aPISST[nX])

			Aadd(aCOFINSAglut,Array(Len(aCOFINS[nX])))
			aCOFINSAglut[Len(aCOFINSAglut)]   :=  Aclone(aCOFINS[nX])

			Aadd(aCOFINSSTAglut,Array(Len(aCOFINSST[nX])))
			aCOFINSSTAglut[Len(aCOFINSSTAglut)]   :=  Aclone(aCOFINSST[nX])

			Aadd(aISSQNAglut,Array(Len(aISSQN[nX])))
			aISSQNAglut[Len(aISSQNAglut)]   :=  Aclone(aISSQN[nX])

			Aadd(aCSTAglut,Array(Len(aCST[nX])))
			aCSTAglut[Len(aCSTAglut)]   :=  Aclone(aCST[nX])

			Aadd(aMedAglut,Array(Len(aMed[nX])))
			aMedAglut[Len(aMedAglut)]   :=  Aclone(aMed[nX])

			Aadd(aArmaAglut,Array(Len(aArma[nX])))
			aArmaAglut[Len(aArmaAglut)]   :=  Aclone(aArma[nX])

			Aadd(aveicProdAglut,Array(Len(aveicProd[nX])))
			aveicProdAglut[Len(aveicProdAglut)]   :=  Aclone(aveicProd[nX])

			Aadd(aDIAglut,Array(Len(aDI[nX])))
			aDIAglut[Len(aDIAglut)]   :=  Aclone(aDI[nX])


			Aadd(aAdiAglut,Array(Len(aAdi[nX])))
			aAdiAglut[Len(aAdiAglut)]   :=  Aclone(aAdi[nX])

			Aadd(aExpAglut,Array(Len(aExp[nX])))
			aExpAglut[Len(aExpAglut)]   :=  Aclone(aExp[nX])

			Aadd(aPisAlqZAglut,Array(Len(aPisAlqZ[nX])))
			aPisAlqZAglut[Len(aPisAlqZAglut)]   :=  Aclone(aPisAlqZ[nX])

			Aadd(aCofAlqZAglut,Array(Len(aCofAlqZ[nX])))
			aCofAlqZAglut[Len(aCofAlqZAglut)]   :=  Aclone(aCofAlqZ[nX])

			nSeq ++
		Else

			If ! Empty(aProd[nX])
				aProdAglut[nPosProd,09] += aProd[nX,09]
				aProdAglut[nPosProd,10] += aProd[nX,10]
				aProdAglut[nPosProd,12] += aProd[nX,12]
				aProdAglut[nPosProd,13] += aProd[nX,13]
				aProdAglut[nPosProd,14] += aProd[nX,14]
				aProdAglut[nPosProd,15] += aProd[nX,15]
				aProdAglut[nPosProd,21] += aProd[nX,21]
			EndIf

			If ! Empty(aICMS[nX])
				aICMSAglut[nPosProd,05] += aICMS[nX,05]
				aICMSAglut[nPosProd,07] += aICMS[nX,07]
				aICMSAglut[nPosProd,09] += aICMS[nX,09]
				aICMSAglut[nPosProd,06] := aICMS[nX,06] // Aliquota do ICMS
			Endif
			...
			If ! Empty(aICMSST[nX])
				...
				//
				If Len( aICMSSTAglut[nPosProd] ) = 0
					aICMSSTAglut[nPosProd]   := Aclone(aICMSST[nX])
				Else
					aICMSSTAglut[nPosProd,05] += aICMSST[nX,05]
					aICMSSTAglut[nPosProd,07] += aICMSST[nX,07]
					aICMSSTAglut[nPosProd,09] += aICMSST[nX,09]
				EndIf

			EndIf

			If ! Empty(aIPI[nX])
				aIPIAglut[nPosProd,06] += aIPI[nX,06]
				aIPIAglut[nPosProd,07] += aIPI[nX,07]
				aIPIAglut[nPosProd,10] += aIPI[nX,10]
			EndIF

			If ! Empty(aPIS[nX])
				aPISAglut[nPosProd,02] += aPIS[nX,02]
				aPISAglut[nPosProd,04] += aPIS[nX,04]
				aPISAglut[nPosProd,05] += aPIS[nX,05]
			EndIf


			If ! Empty(aPISST[nX])
				aPISSTAglut[nPosProd,02] += aPISST[nX,02]
				aPISSTAglut[nPosProd,04] += aPISST[nX,04]
				aPISSTAglut[nPosProd,05] += aPISST[nX,05]

			EndIf

			If ! Empty(aCOFINS[nX])
				aCOFINSAglut[nPosProd,02] += aCOFINS[nX,02]
				aCOFINSAglut[nPosProd,04] += aCOFINS[nX,04]
				aCOFINSAglut[nPosProd,05] += aCOFINS[nX,05]
			EndIf

			If ! Empty(aCOFINSST[nX])
				aCOFINSSTAglut[nPosProd,02] += aCOFINSST[nX,02]
				aCOFINSSTAglut[nPosProd,04] += aCOFINSST[nX,04]
				aCOFINSSTAglut[nPosProd,05] += aCOFINSST[nX,05]
			EndIf

			If ! Empty(aISSQN[nX])
				aISSQNAglut[nPosProd,02] += aISSQN[nX,02]
				aISSQNAglut[nPosProd,05] += aISSQN[nX,05]
			EndIf

		EndIf
	Next

	For nLoop :=1 to Len(aICMSAglut)
		aICMSAglut[nLoop,07] := (aICMSAglut[nLoop,05] * (aICMSAglut[nLoop,06]/100))
	Next


	aProd		:= aProdAglut
	aICMS		:= aICMSAglut
	aICMSST		:= aICMSSTAglut
	aIPI		:= aIPIAglut
	aPIS		:= aPISAglut
	aPISST		:= aPISSTAglut
	aCOFINS		:= aCOFINSAglut
	aCOFINSST	:= aCOFINSSTAglut
	aISSQN		:= aISSQNAglut
	aCST		:= aCSTAglut
	aMed		:= aMedAglut
	aArma		:= aArmaAglut
	aveicProd	:= aveicProdAglut
	aDI			:= aDIAglut
	aAdi		:= aAdiAglut
	aExp		:= aExpAglut
	aPisAlqZ	:= aPisAlqZAglut
	aCofAlqZ	:= aCofAlqZAglut
...
*/
	...
	*-----------------------------*
Static Function MontaTexto(cRet)
	*-----------------------------*
	Local cfunc
	Local bFunc
	Local cTexto    :="", nPos
	...
	While nPos<>0
		...
		cFormula:=Substr(cRet,1,nPos-1)
		cFunc:= "{||"+cFormula+"}"
		bFunc:=&cFunc
		cTexto:=Alltrim(EVAL(bFunc)) + ' '

		//  cTextoRet+= Padr(cTexto,MAXMENLIN)
...
Static Function AddMsgTxt(aMsg,cTexto,cTxtBusca,cAlias,cTipo)
	...
	While  nPos<>0
		...
		cTxtAux:=Substr(cTexto,1,nPos-1)   // "DESCONTO DE 7% CONF. INCISO III DO ART. 84 ANEXO I RICMS 45.490 DE 30/11/2000 #7.00#"
		// "DESCONTO DE 1,65% CONF. ART. 1o PORT 162 DE 06/06/2005 DA SUP. DA ZONA FRANCA #1.65#"
		// "DESCONTO DE 7,60% CONF. ART. 1o PORT 162 DE 06/06/2005 DA SUP. DA ZONA FRANCA #7.60#"
		cPerc  :=Substr(cTexto,++nPos,4)

		nPosTxt:=Ascan(aMsg,{|x|x[1]==cPerc})

		nVal:=0

		If cTipo == "1" // Saida
			If cPerc == "7.00" .and. ! Empty((cAlias)->D2_DESCZFR)
				nVal:=(cAlias)->D2_DESCZFR
			Endif

			If cPerc == "1.65" .and. ! Empty((cAlias)->D2_DESCZFP)
				nVal:=(cAlias)->D2_DESCZFP
			Endif

			If cPerc == "7.60" .and. ! Empty((cAlias)->D2_DESCZFC)
				nVal:=(cAlias)->D2_DESCZFC
			Endif
		ElseIf cTipo == "2"  // Entrada
			If cPerc == "7.00" .and. ! Empty((cAlias)->D1_DESCZFR)
				nVal:=(cAlias)->D1_DESCZFR
			Endif

			If cPerc == "1.65" .and. ! Empty((cAlias)->D1_DESCZFP)
				nVal:=(cAlias)->D1_DESCZFP
			Endif

			If cPerc == "7.60" .and. ! Empty((cAlias)->D1_DESCZFC)
				nVal:=(cAlias)->D1_DESCZFC
			Endif


		Endif

		If nPosTxt == 0 .and. nVal <> 0
			Aadd(aMsg,{cPerc,cTxtAux,nVal})
			//     nPosTxt:=Len(aTextoAdic)
		Else
			If nVal <> 0
				aMsg[nPosTxt,3]+=nVal
